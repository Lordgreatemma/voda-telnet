	...
	class unityContentFunctions extends db_connect
	{
		...
		public function check_duplicates($today_date, $serviceName)
		{
			...
			{	
				...
				$query =  $this->db_conn->query("SELECT serviceName, billedCount FROM `daily_reports` WHERE serviceName = '$serviceName' AND DATE(`created_at`) = '$today_date' LIMIT 0,1");//(year(created_at) = year(now()) AND month(created_at) = month(now()) AND day(created_at) = day(now())) DATE(`created_at`) = '$today_date'
				$query->execute();
		...
		public function check_billed_duplicates($today_date = null)
		{
			try 
			{
				...
				$query =  $this->db_conn->query("SELECT * FROM `daily_billed` WHERE DATE(`created_at`) = '$today_date' ");// DATE(`entry_date`) = '$today_date'year(created_at) = year(now()) AND month(created_at) = month(now()) AND day(created_at) = day(now())
				$query->execute();
				$counts = $query->rowCount();
				return $counts;	
			} catch (Exception $e)
		...
		public function get_keyword_counts($serviceName)
		{
			try 
			{
				$stmt = "SELECT msisdn FROM mt_subscribers WHERE serviceName = '$serviceName'";//limit 50
		...
		public function get_sms_content_counts($offerName, $today_date)
		{
			try 
			{
				...
				$stmt  = "SELECT msisdn FROM mt_charge WHERE offerName = '$offerName' AND DATE(`created_at`) = '$today_date' ";//(year(created_at) = year(now()) AND month(created_at) = month(now()) AND day(created_at) = day(now()))
		        $query = $this->db_conn->prepare($stmt);
		        $query->execute();
	        	return $query->rowCount();
			}catch (Exception $e){
		...
		public function get_sms_delivery_counts($offerName, $today_date)
		{
			try 
			{
				...
				$stmt  = "SELECT msisdn FROM mt_charge WHERE (offerName = '$offerName' AND transactionId != '' ) AND DATE(`created_at`) = '$today_date' ";//(year(created_at) = year(now()) AND month(created_at) = month(now()) AND day(created_at) = day(now()))
		        $query = $this->db_conn->prepare($stmt);
		        $query->execute();
	        	return $query->rowCount();
			}catch (Exception $e){
...
//SELECT DISTINCT(offerName) AS keyword, SUM(amount) AS amount, COUNT(msisdn) AS counts FROM mt_charge WHERE transactionId != '' AND DATE(created_at) = "2022-05-29" GROUP BY keyword
		...
		public function get_count_subscribers($keyOne, $keyTwo)
		{
			try 
			{			
				$sql   = "SELECT msisdn FROM mt_subscribers WHERE serviceName = '$keyOne' OR serviceName = '$keyTwo' ";
		...
		public function fetch_total_content_sent_today()
		{
			...
			{	
				...
				$sql = "SELECT msisdn FROM mt_charge WHERE  DATE(`created_at`) = '$today_date'";//(year(created_at) = year(now()) AND month(created_at) = month(now()) AND day(created_at) = day(now()))
		        $query = $this->db_conn->prepare($sql);
				$query->execute();
				return $query->rowCount();
			} catch (Exception $exc) 
		...
		public function fetch_today_delivery($today = null)
		{
			...
			{	
				...
				$sql = "SELECT msisdn FROM mt_charge WHERE transactionId != '' AND DATE(`created_at`) = '$today_date'";//(year(created_at) = year(now()) AND month(created_at) = month(now()) AND day(created_at) = day(now())) 
		        $query = $this->db_conn->prepare($sql);
				$query->execute();
				return $query->rowCount();
			} catch (Exception $exc) 
