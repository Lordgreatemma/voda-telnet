...
//https://mysmsinbox.com/telnet_sdp/mt/v1/unsub-mt-service.php
	...
	if($_SERVER['REQUEST_METHOD'] == 'POST'){
		if(empty($_POST['accountId'])){
			...
			die();
		}elseif(!is_numeric($_POST['accountId']) || trim(strlen($_POST['accountId'])) != 12) {
		...
		{
			$unsubResponse['message'] = "Provide shortCode for the unsubscription.";
			header('Content-type: application/json; charset=utf-8');
			echo json_encode($unsubResponse);
			die();
		}elseif(empty($_POST['accountIdType'])) {
			$unsubResponse['message'] = "Provide accountIdType to be unsubscribed, eg. MSISDN.";
			header('Content-type: application/json; charset=utf-8');
			echo json_encode($unsubResponse);
			die();
		}
		elseif(empty($_POST['carrierId'])) {
			$unsubResponse['message'] = "Provide the carrierId.";
			header('Content-type: application/json; charset=utf-8');
			echo json_encode($unsubResponse);
			die();
		} 
		...
		{			
			$resp = $modelObj->deleteCustomerAccount($_POST['carrierId'], $_POST['accountIdType'], $_POST['shortCode'], $_POST['accountId']);
			echo $resp;
		}
