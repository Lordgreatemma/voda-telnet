...
set_time_limit(13600);
	...
	if ($data) {
		...
		$Batch_4Data = $dataObj->getWisdomSubscribersBatch_4($offerName, $offerId);
		if($Batch_4Data){
			foreach ($Batch_4Data as $key => $value_4s) {
				$dataContObj->sendContent($value_4s['msisdn'], $offerId, $message, $offerName, $clientCorrelator);
			}
		} 
	}else{
...
// echo $dataContObj->sendContent('233206846412', '1402', 'text message', $offerName, $clientCorrelator);
	...
	// $createdDate = date("Y-m-d");
	// $file        = fopen("logs/cron/WSDMDailyRun4.log",'a');
	// $values      = "[  $ends ];\n";
	// fwrite($file, "$values");
	// fclose($file);
?>